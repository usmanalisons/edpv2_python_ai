...
async def get_document_search_logic(request: Request):
    ...
    chat_memory_manager = request.app.state.chat_memory_manager
    intranet_db_service = request.app.state.intranet_db_service
    quantum_db_service = request.app.state.quantum_db_service

    return DocumentSearchLogic(
        chroma_db_service=chromadb_service,
        memory_manager=chat_memory_manager,
        intranet_db_service=intranet_db_service,
        quantum_db_service=quantum_db_service,
    )
...
@router.get("/chat/messages/{id}")
async def get_chat_messages(id: str, logic: DocumentSearchLogic = Depends(get_document_search_logic)):
